...
if [ "$(getprop persist.magisk.installed)" = "1" ]; then
    exit 0
fi

if ! pm list packages | grep -q "com.topjohnwu.magisk"; then
    if [ -f "/data/magisk.apk" ]; then
        if [ "$(getprop ro.build.version.sdk)" -le 28 ]; then
            if ! pm install -r "/data/magisk.apk"; then
                cp "/data/magisk.apk" "/data/app/com.topjohnwu.magisk.apk"
                chmod 644 "/data/app/com.topjohnwu.magisk.apk"
